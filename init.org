...
#+begin_src emacs-lisp
  ...
  (leaf *deepl
    ...
    (cl-defun my:deepl-output-message (&key text &allow-other-keys)
...
#+begin_src emacs-lisp
  (leaf tempel
    ...
    (tempel-map
     ...
     ("C-," . tempel-previous))
    :hook
    (after-change-major-mode-hook . my:insert-commit-template-on-magit)
    :custom
    ;; トリガーは>にしておく。<だとorgの方と被ってしまうため、ここは回避しておくこととする。
...
#+begin_src emacs-lisp
  ...
  (leaf simple-pomodoro
    ...
    (leaf simple-pomodoro-mode-line
      :require t
      :config
      (setq simple-pomodoro-tick-function (lambda (&rest args) (simple-pomodoro-mode-line-update-text)))))
#+end_src
...
#+begin_src emacs-lisp
  (leaf corfu
    ...
    (corfu-auto-delay . 0)                 ;; 即時表示を試してみる
    (corfu-count . 15)                        ;; show more candidates
    (corfu-auto-prefix . 2)
    (corfu-max-width . 250)               ;; max width of corfu completion UI

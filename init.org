...
*** tempel
tempoというパッケージのシンタックスを利用してmodernizeしたもの。tempo自体は1995年からずっと利用されており、そのシンタックスやシンプルさとのバランスに定評がある。

ちょっと注意なのが、yasnippetみたいに、展開しおわったときに最後にいる場所、みたいなことができないので、展開したなかで色々修正したりなんだりしないといけない、という点がある。
後、最後までいかないと終了できない。なので、どちらかというと途中途中で編集しながらなんとかする、みたいな感じになる。

#+begin_src emacs-lisp
  (leaf tempel
    :straight (tempel :type git :host github :repo "minad/tempel")
    :hook
    (prog-mode-hook . my:tempel-setup-capf)
    (text-mode-hook . my:tempel-setup-capf)
    :preface
    ...
    (defun my:tempel-active-p ()
      "If tempel is active, return T"
      tempel--active)

    (defun my:stop-activity-if-tempel-active (f &rest args)
      "Stop auto-save-buffers-enhanced activity if tempel is active"
      (unless (my:tempel-active-p)
        (apply f args)))

    (advice-add 'auto-save-buffers-enhanced-save-buffers
                :around
                #'my:stop-activity-if-tempel-active))
#+end_src
...
#+begin_src emacs-lisp
  (leaf shackle
    ...
    (shackle-rules . '((compilation-mode :align t :size 0.4)
                       (flycheck-error-message-mode :align t :size 0.3)
                       (special-mode :align t :size 0.3)))
...
*** super-save
auto-save-buffers-enhancedがうまいこと更新されないようになってしまったので、とりあえず一番広く利用されていそうなものを利用することにした。
...
#+begin_src emacs-lisp
  (leaf lsp-mode
    ...
    (defun my:lsp-mode-setup-completion ()
      ;; add tempel into capf that lsp is resetted

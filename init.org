...
#+begin_src emacs-lisp
  ...
  "
    ...
    (defun my:modalka-setup ()
      ...
      ;; buffer
      (define-key modalka-mode-map ";" #'switch-to-buffer)
...
#+begin_src emacs-lisp
  (setup perfect-margin
    ...
    (:hook-into emacs-startup-hook)
    (:when-loaded
      (setopt perfect-margin-disable-in-splittable-check t)
  ...
  (setup tab-bar
    ...
    (defun my:tab-name-format-function (tab i)
      ...
      (let ((padding 8))
        (concat
         ...
         (apply #'tab-bar-tab-name-format-default (list tab i))
         (propertize " "
                     'display `((space :width (,padding)))
                     'face (funcall tab-bar-tab-face-function tab))
         )))
    ...
    (:when-loaded
      ...
      (set-face-attribute 'tab-bar nil
                          ...
                          ':weight 'light)
      (setopt tab-bar-tab-name-format-function #'my:tab-name-format-function)
      (setopt tab-bar-separator 'my:tab-bar-separator)
...
#+begin_src emacs-lisp
  (setup tabspaces
    ...
    (:when-loaded
      ...
      (setopt tabspaces-auto-restore t)
      ))
#+end_src
** モードライン関連
    ...
    (defun my:corfu--setup:around (oldfun &rest args)
      (if my:corfu-in-auto-complete
          ...
          (progn
            (setf
             ;; 子フレームを半透明にする
             (alist-get 'alpha-background corfu--frame-parameters) 90
             ;; 最初の候補を選ばない

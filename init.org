...
#+begin_src emacs-lisp
  (leaf dired
    ...
    :config
    ;; macOSの場合、lsがcoreutilsとは別物なので、lsを使わないようにする
    (when (eq system-type 'darwin)
...
*** volatile-highlights
一時的に変更した部分(yank/killとか)をハイライトするだけのパッケージ。視覚的なフィードバックが得られる。
...
*** doom-modeline
doom emacsで使われているmode lineのパッケージ。
...
#+begin_src emacs-lisp
  (leaf *my-mode-line
    :after moody
    :init
    (defface  my:buffer-position-active-face
      '((t (:inherit 'mode-line
                     :background "bg-graph-red-1"
                     :foreground "red-warmer")))
      "Face for active buffer position indicator."
      :group 'my:mode-line)

    (defface my:buffer-position-inactive-face
      '((t (:inherit 'mode-line-inactive
                     :background "bg-inactive"
                     :foreground "red-warmer")))
      "Face for inactive buffer position indicator."
      :group 'my:mode-line)
      ...
      (cond 
       ((vc-backend buffer-file-name)
        (let ((branch-name (car (vc-git-branches)))
              (state (cl-case (vc-state buffer-file-name)
                       ('added "  ")
                       ('needs-merge "  ")
                       ('needs-update "  ")
                       ('removed "  ")
                       (t "  "))))
          (concat state branch-name)))
       (t "")))
    ...
    (setq my:mode-line-element-buffer-status '(:eval (my:mode-line-status))
          my:mode-line-element-skk-indicator '(:eval (if (string= "" skk-modeline-input-mode)
                                                         ""
                                                       (moody-tab skk-modeline-input-mode 1 'up)))
          my:mode-line-element-major-mode '(:eval (concat " " (let ((name mode-name))
                                                                (cond
                                                                 ((consp name) (car name))
                                                                 (t name)))
                                                          " "))
          my:mode-line-element-vc-mode '(:eval (moody-ribbon (my:mode-line-vc-state)))
          my:mode-line-element-buffer-position '(:eval (moody-ribbon (my:mode-line-buffer-position-percentage) 5 'up
                                                                     'my:buffer-position-active-face
                                                                     'my:buffer-position-inactive-face
                                                                     )))
    ...
    (put 'my:mode-line-element-buffer-position 'risky-local-variable-p t)
    ...
    (setq-default mode-line-format
                  '("%e"
                    ...
                    (vc-mode my:mode-line-element-vc-mode)
...
#+begin_src emacs-lisp
  (leaf migemo
    :straight t
    :require t
    :if (executable-find (or my:migemo-command ""))

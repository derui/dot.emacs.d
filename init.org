...
#+begin_src emacs-lisp
  (leaf *global-user
    ...
    ;; for org-mode
    (define-key my:global-user-mode-keymap (kbd "o") #'my:org-transient)
    ;; shortcut for major functions
    ;; view
    (define-key my:global-user-mode-keymap (kbd "l") #'my:view-transient)
    ;; navigation
    (define-key my:global-user-mode-keymap (kbd "n") #'my:navigation-transient)
    ...
    (define-key my:global-user-mode-keymap (kbd "v") #'my:persp-transient)

    (global-set-key "<escape>" my:global-user-mode-keymap)
    )
#+end_src
...
#+begin_src emacs-lisp
  (leaf *save-buffer
    ...
    (defun my:save-buffer-advice (f &rest args)
      "save-bufferを非同期で実行する"
...
#+begin_src emacs-lisp
  (defvar my:display-buffer-list-in-side-window
    `((0 . ,(rx (or
                 "*completion*"
                 "*Help*"
...
*** navigation
移動に関するcommandをまとめたtransient。検索はnavigationというわけではないので、ここでは定義しない。

#+begin_src emacs-lisp
  (leaf *navigation-transient
    :after (transient)
    :config
    (transient-define-prefix my:view-transient ()
      "Prefix for view sources/documents with some copies"
      [
       ["Basic move"
        ("h" "Backward character" backward-char :transient t)
        ("j" "Next line" next-line :transient t)
        ("k" "Previous line" previous-line :transient t)
        ("l" "Forward character" forward-char :transient t)
        ("w" "Forward word" forward-word :transient t)
        ("b" "Backward word" backward-word :transient t)
        ("e" "End of line" move-end-of-line :transient t)
        ("a" "Beginning of line" move-beginning-of-line :transient t)
        ("SPC" "Set mark" set-mark-command :transient t)
        ]
       ["Scroll"
        ("d" "Scroll down" scroll-down :transient t)
        ("u" "Scroll up" scroll-up :transient t)
        ]
       ["Jump"
        ("g" "Goto line" goto-line :transient t)
        ("t" "Avy goto char" avy-goto-char)
        ]
       ]
      ))
#+end_src
...
#+begin_src emacs-lisp
  (leaf *mark/replace-transient
    ...
    (defun my:mark-whole-line ()
      "Mark whole current line at point"
      (save-excursion
        (beginning-of-line)
        (set-mark (point))
        (end-of-line)))
    ...
    (transient-define-prefix my:mark/replace-transient ()
      ...
      [
       ["Mark"
        ("s" "Mark sexp thing at point" puni-mark-sexp-at-point :transient t)
        ("(" "Mark list around point" puni-mark-list-around-point :transient t)
        ("S" "Mark sexp around point" puni-mark-sexp-around-point :transient t)
        ("e" "Expand mark" puni-expand-region :transient t)
        ("l" "Mark whole line" my:mark-whole-line :transient t)
        ("v" "View" my:view-transient :transient t)
        ]
       ["Rectangle"
        ("r" "Mark rectangle" rectangle-mark-mode :transient t)
        ("k" "Kill rectangle" rectangle-kill)
        ("y" "Copy rectangle" copy-rectangle-as-kill)
        ]
       ["Narrow/Widen"
        ("n" "Narrow to region" narrow-to-region :transient t)
        ("w" "Widen" widen :transient t)
        ]
       ["Replace"
  ...
  (leaf *persp-transient
    ...
    (transient-define-prefix my:persp-transient ()
      ...
      [
       ...
       ["Change perspective"
        ("v" "Switch perspective" persp-switch)

...
(with-low-priority-startup
 (cl-defun
  my:treesit-expand-region--between-node
  (&key start end)
  "`(START END)' の間に存在するnodeを取得する"
...
(declare-function 'my/ace-window-one-command-by nil)

(with-low-priority-startup
 (defvar my/split-window-right
   (my/ace-window-one-command-by (lambda () (split-window-right)))
   "Split the current window rightwards and do a command")

 (defvar my/split-window-below
   (my/ace-window-one-command-by (lambda () (split-window-below)))
   "Split the current window below and do a command.")
 ...
 (transient-define-prefix
  ...
  [["Basic navigations"
   ...
   ["Split window" ("s" "Split vertically" split-window-vertically)
    ...
    ("S" "Split vertically and switch to other" my/split-window-below)
    ("V"
...
(defun my/setup-header-line (&rest _)
  ...
  (set-face-attribute 'header-line-inactive nil
                      ...
                      `(:line-width
                        (8 . 8)
                        :color
                        ,(face-background 'header-line-inactive nil t)
                        :style nil))
...
(with-low-priority-startup
 (my/setup-header-line)

 (add-hook 'enable-theme-functions #'my/setup-header-line))
...
(with-eval-after-load 'multistate
  ...
  (set-key! multistate-normal-state-map "4" my/split-window-below)
...
(with-eval-after-load 'rust-ts-mode (setopt rust-ts-indent-offset 4))
...
(defun my/ace-window-one-command-by
    (command &optional stay-current-window)
  "Return a new interactive function.

It will run a `COMMAND' in the selected window using `ace-window'.

If `STAY-CURRENT-WINDOW' is non-nil, stay current window after run command.
When it is nil or not passed, run `select-window' with returned window by `command'."
